...
def detect(cap, stframe, output_file_name, save_output, model_players, model_keypoints,
    ...
    tac_map = cv2.imread('../tactical map.jpg') # dimensions of the image are: 314 x 546p, and the top left is 0,0
            ...
            if 'homog' in locals():
                ...
                if detected_ball_src_pos is not None:
                    ...
                    detected_ball_dst_pos = np.transpose(dest_point)
                     # Log ball position to the file
                    ball_position_file.write(f"{frame_nbr},{detected_ball_dst_pos[0]},{detected_ball_dst_pos[1]}\n")
                    print(f"ball position: {detected_ball_dst_pos}") # ball positon relative to the field
